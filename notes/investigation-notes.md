# Investigation Notes — Lab 59 Suspicious DLL Name Investigation

## Investigation Overview

The investigation examined files using names associated with legitimate Windows DLLs. The purpose was to determine whether the filenames corresponded to genuine Windows components or merely represented misleadingly named files.

All artifacts were created safely as text files inside:

`C:\SuspiciousDLLNameLab`

The files were not executed.
...
1. Three DLL-style filenames were created.
2. All three files were harmless text files.
3. Their sizes were extremely small compared with legitimate Windows DLLs.
4. The controlled files had different SHA-256 values from the real Windows DLLs.
5. Legitimate `kernel32.dll` and `version.dll` were Authenticode-valid.
6. Controlled `kernel32.dll` and `version.dll` returned `UnknownError`.
7. Sysmon Event ID 1 was available.
8. Sysmon Event ID 3 was available.
9. Sysmon Event ID 7 was unavailable.
10. No relevant PowerShell Event ID 4104 was identified.
11. No relevant Security Event ID 4688 was identified.
12. No controlled DLL was executed or loaded.
13. No relevant DLLs were found in Temp, Downloads, or LocalAppData during the searches performed.

---

# Analyst Assessment

The controlled files demonstrate how a suspicious DLL name can create an appearance of legitimacy without actually being the legitimate Windows component.
...
clearly distinguishes the controlled artifacts from the legitimate Windows DLLs.

However, execution or loading of the test files was never established.

---

# Final Investigation Conclusion

The investigation successfully demonstrated DLL-name masquerading characteristics without executing the test files.

The evidence strongly establishes that the controlled artifacts were not the legitimate Windows DLLs they were named to resemble.

The available telemetry does not establish DLL loading or execution.

The correct DFIR conclusion is therefore:

> The artifacts exhibited suspicious DLL naming and masquerading characteristics, but there is no evidence from this controlled lab that the files were executed or loaded.
